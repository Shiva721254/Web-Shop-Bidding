<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
      ...
      <select v-model="typeFilter" class="border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
    ...
    <div v-else-if="error" class="flex flex-col items-center py-24 gap-4">
      ...
      <button @click="fetchProducts" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Try Again</button>
    </div>

    <div v-else>
      <div v-if="products.length === 0" class="text-center py-16 text-gray-500">No products found.</div>
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="product in products" :key="product.id"
          class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow flex flex-col">
          <div class="flex justify-between items-start mb-3">
            <span class="px-2 py-1 bg-gray-100 text-gray-600 text-xs font-medium rounded-full">{{ product.category }}</span>
          ...
          <div class="flex justify-between items-center mt-auto">
            <span class="text-xl font-bold text-blue-700">€{{ (product.price ?? product.startingPrice)?.toFixed(2) }}</span>
            <RouterLink v-if="product.type === 'auction'" :to="{ name: 'AuctionDetail', params: { id: product.id } }"
...
async function fetchProducts() {
  ...
  try {
    ...
    totalPages.value = Math.ceil((body.total ?? products.value.length) / limit)
  } catch (err) {
