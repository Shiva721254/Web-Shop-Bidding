<template>
  <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div v-else-if="orders.length === 0" class="text-center py-16 text-gray-500">
      <p class="text-lg mb-4">You have no orders yet.</p>
    ...
    <div v-else class="space-y-4">
      <div v-for="order in orders" :key="order.id" class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="border-t border-gray-100 pt-4">
          <ul class="space-y-2">
            <li v-for="item in order.items" :key="item.id" class="flex justify-between text-sm text-gray-600">
...
const statusClass = (s) => ({ pending: 'bg-yellow-100 text-yellow-700', confirmed: 'bg-green-100 text-green-700', cancelled: 'bg-red-100 text-red-700' }[s] ?? 'bg-gray-100 text-gray-600')
...
async function fetchOrders() {
  ...
  try {
    ...
    const res  = await get(`/users/${userId}/orders`)
