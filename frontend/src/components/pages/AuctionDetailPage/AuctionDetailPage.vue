    ...
    <div v-else-if="auction">
      <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 mb-6">
        <div class="flex items-start justify-between mb-4">
          <span class="px-3 py-1 bg-blue-100 text-blue-700 text-sm font-medium rounded-full">{{ auction.category }}</span>
        ...
        <div v-if="auction.status === 'open'" class="border-t border-gray-100 pt-6">
          ...
          <form v-else @submit.prevent="placeBid" class="flex gap-3">
            <input v-model.number="bidAmount" type="number" step="0.01" :min="minBid"
...
async function placeBid() {
  ...
  try {
    ...
    bidSuccess.value         = `Bid of €${bidAmount.value.toFixed(2)} placed!`
